...
class _TypedNetwork:
    ...
    def add_elementwise(self, lhs, rhs, operation):
        assert lhs.dtype == rhs.dtype, (
            f"strongly typed {operation}: {lhs.dtype} != {rhs.dtype}"
        )
...
def test_normalization_uses_runtime_dtype_for_fp32_compute(
    ...
    if normalization == "rms":
        ...
        result = ops.add_rms_norm_per_head(
            network, inp, 2, 2, gamma, epsilon, dtype=storage_dtype
        )
    else:
        result = ops.add_layer_norm(
            network, inp, 4, gamma, np.zeros(4, dtype=storage_dtype), epsilon,
            dtype=storage_dtype,
        )
