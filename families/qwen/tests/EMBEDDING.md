...
`--e2e-model qwen3-embedding-0.6b`; it compares native output with Transformers
using cosine >= 0.99, L2 distance <= 0.1, and unit-norm error <= 0.001. Build the
`qwen_embedding_consumer` CMake target and provide the normal Qwen E2E runtime
and CUDA environment. CPU contract tests and protocol fixtures do not qualify
